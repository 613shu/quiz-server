...
// החלטה עסקית (8.8.2026): גישה מורחבת נקבעת אך ורק לפי תשלום אמיתי
// (paidUntil), בשום מצב לא לפי מי הפעיל את המשחק - כולל מנהל-על. משתמשים
// בפונקציה אחת משותפת כדי שלא יהיה שוב פער בין route לroute.
function computeHasExtendedAccess(game) {
    return !!(game.paidUntil && game.paidUntil > new Date());
}
...
router.get('/', async (req, res) => {
    ...
    res.json(games.map((g) => ({
        ...
        hasExtendedAccess: computeHasExtendedAccess(g),
...
router.post('/:gameId/activate', requireGameOwnership, async (req, res) => {
    ...
    // החלטה עסקית (8.8.2026): adminActivated נשאר לתיעוד בלבד ("מי הפעיל
    // אחרון") - לא פותח יותר גישה מלאה חינם, גם כשמנהל-על מפעיל. גישה מורחבת
    // אך ורק דרך paidUntil (ראו computeHasExtendedAccess למעלה).
...
// ===== שלב 5: סטטוס גישה (חינם/בתשלום) של המשחק - per-game. נועד להצגה =====
// ב-admin.html (מסך חי משותף) - שם רוצים לדעת מה המצב האמיתי של הסשן הנוכחי.
router.get('/:gameId/access-status', requireGameOwnership, async (req, res) => {
    res.json({
        maxFreePlayers: CONFIG.FREE_TRIAL_MAX_PLAYERS,
        hasExtendedAccess: computeHasExtendedAccess(req.game),
