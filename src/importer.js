    ...
    do {
      ...
      for (const rec of reqs) {
        ...
        try { report = await runImport(provider, rec, opts); }
          ...
          // תקלה זמנית (חיבור שנפל וכו') – ננסה שוב אוטומטית עד 3 פעמים. בקשה שלא הושלמה לא מסומנת כמעובדת.
          provider.importTries = provider.importTries || new Map();
          const tries = (provider.importTries.get(rec.uid) || 0) + 1;
          provider.importTries.set(rec.uid, tries);
          const retry = tries < 3;
          if (retry) setTimeout(() => { provider.importSeen.delete(rec.uid); processImports(provider, opts).catch(() => {}); }, 60e3 * tries);
          report = { at: new Date().toISOString(), subject: rec.subject, from: rec.from.address, added: 0, dup: 0, failed: 0, labels: [], unknown: [],
            error: `הייבוא נכשל: ${e.message}` + (retry ? ` – ינוסה שוב אוטומטית בעוד ${tries} דק׳` : ' – נכשל 3 פעמים. אפשר לנסות שוב על ידי הפעלה מחדש של השרת') };
        }
